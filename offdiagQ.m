function sum=offdiagQ(H)
% sum of the off diagonnal elements of the quaternion matrix
...
%
% Sum is the lower    triangular of H0  under the subdiagonal 
...
if m>1
    for k=2:m
        sum=sum+norm(H0(k,1:k-2))^2+norm(H1(k,1:k-1))^2+norm(H2(k,1:k-1))^2+norm(H3(k,1:k-1))^2;
%         for t=1:k-1
