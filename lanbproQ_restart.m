function [U,B_k,V,p,ierr,work] = lanbproQ_restart(varargin)

%LANBPROQ_RESTART Lanczos bidiagonalization with partial reorthogonalization.
%   LANBPROQ_RESTART computes the Lanczos bidiagonalization of a quaternion
%   matrix using the  with partial reorthogonalization.
%
%   [U_k,B_k,V_k,R,ierr,work] = LANBPROQ_RESTART(A,K,R0,OPTIONS,U_old,B_old,V_old)
%
...
if isempty(U)
    ...
    id=j; 
%     disp('Bk='),full(B_k),id,j,size(B_k),size(U),size(V)
    Bk1rho_i=B_k(id,1:(id-1)); % the last row of B_k, where rho_i saved.
...
if ~exU
    B_k(id,1:id-1)=Bk1rho_i;% save rho_i into B_k
end
