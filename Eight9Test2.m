...
% Reconstruct the reduced-rank quaternion matrix
quaternion_matrix = quaternion_matrix0;


    [n, m] = size(quaternion_matrix);
[rows, cols] = size(quaternion_matrix);
    norms = zeros(1, cols);
    
    for i = 1:cols
        norms(i) = norm(quaternion_matrix(:, i));
    end
    
    % Sort the columns based on the norm values in descending order
    [~, sorted_indices] = sort(norms, 'descend');
    
    % Reorder the columns of the quaternion matrix
    sorted_matrix = quaternion_matrix(:, sorted_indices);
quaternion_matrix=sorted_matrix;
