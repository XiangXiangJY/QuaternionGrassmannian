% ------------ STEP 1: Combine quaternion vectors (KTH-TIPS2-a) ------------

base_directory = '/Users/xiangxiang/Documents/MATLAB/Code fo MTH80 Quaternion Grassmannian Classification/KTH-TIPS2-a';
class_names = {'aluminium_foil', 'brown_bread', 'corduroy', 'cork', 'cotton', ...
...
for class_idx = 1:length(class_names)
    ...
    sample_dirs = dir(class_dir);
    sample_dirs = sample_dirs([sample_dirs.isdir] & ~startsWith({sample_dirs.name}, '.'));

    for s = 1:length(sample_dirs)
        sample_path = fullfile(class_dir, sample_dirs(s).name);
        image_files = dir(fullfile(sample_path, '*.png'));  % or '*.jpg'
        ...
        tag = sprintf('%s_%s', class_names{class_idx}, sample_dirs(s).name);
...
[~, idx] = sort({projector_files.name});
projector_files = projector_files(idx);
...
disp('Distance matrix (first 10×10):');
disp(distance_matrix(1:10, 1:10));
